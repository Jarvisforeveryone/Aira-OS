package com.aira.assistant.core.memory
...
object MemoryManager {
    ...
    fun setupCrashGuard(context: Context) {
        ...
        // GUARD: Only install the handler ONCE. Never re-install on every Activity.
        if (handlerInstalled) return
        handlerInstalled = true

        // Capture ONLY the application context — this never leaks Activity.
        val safeAppContext = context.applicationContext
        val defaultHandler = Thread.getDefaultUncaughtExceptionHandler()
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            try {
                Logger.e(TAG, "Uncaught exception caught by Safety Guard!", throwable)
                val prefs = safeAppContext.getSharedPreferences(PREFS_SAFETY, Context.MODE_PRIVATE)
    ...
    /**
     * Trims application memory and triggers garbage collection.
     */
    fun trimMemory(context: Context) {
        Logger.i(TAG, "MemoryManager: Trim memory requested. Triggering garbage collection.")
        System.gc()
    }
    ...
    fun isLlamaSupported(context: Context): Boolean {
        return try {
            val is64Bit = android.os.Build.SUPPORTED_ABIS.any { it.contains("64") }
            if (!is64Bit) {
                Logger.w(TAG, "32-bit architecture detected without native 64-bit support. Disabling heavy models for stability.")
                return false
            }
            ...
            Logger.d(TAG, "Device total RAM: ${totalRam / (1024 * 1024)} MB, isLowRamDevice: $isLowRam, is64Bit: $is64Bit")
