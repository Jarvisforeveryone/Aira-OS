package com.aira.assistant.network.providers

import android.content.Context
import android.util.Log
import com.aira.assistant.data.AppDatabase
import com.aira.assistant.data.GroqCache
import com.aira.assistant.data.Memory
import com.aira.assistant.data.QueryCache
import com.aira.assistant.network.api.ApiProvider
import com.aira.assistant.network.api.ProviderManager
...
object JarvisEmotionEngine {
    ...
    fun detectEmotion(text: String, provider: ApiProvider? = null): EmotionState {
        val detected = com.aira.assistant.utils.EmotionDetector.detectEmotion(text)
        val emotion = when (detected.emotion) {
            com.aira.assistant.utils.UserEmotion.HAPPY -> UserEmotion.HAPPY
            com.aira.assistant.utils.UserEmotion.SAD -> UserEmotion.SAD
            com.aira.assistant.utils.UserEmotion.ANGRY -> UserEmotion.ANGRY
            com.aira.assistant.utils.UserEmotion.NEUTRAL -> UserEmotion.NEUTRAL
        }
        lastEmotion = emotion
...
sealed class SlotFillResult {
    data class Complete(val command: com.aira.assistant.utils.ParsedCommand) : SlotFillResult()
    data class Incomplete(val missingSlotPrompt: String, val partialCommand: com.aira.assistant.utils.ParsedCommand) : SlotFillResult()
}

object JarvisSlotFiller {
    fun evaluate(parsed: com.aira.assistant.utils.ParsedCommand?): SlotFillResult? {
        if (parsed == null) return null
        return when (parsed.type) {
            com.aira.assistant.utils.CommandType.SET_ALARM -> {
            ...
            com.aira.assistant.utils.CommandType.LAUNCH_APP -> {
...
data class DAGStep(
    ...
    val parsedCommand: com.aira.assistant.utils.ParsedCommand?
)

object JarvisWorkflowDAG {
    fun parseMultiStepInput(input: String, brightness: Int = 50): List<DAGStep> {
        val parts = com.aira.assistant.utils.InstantMultiCommandDispatcher.splitIntoSubCommands(input)

        if (parts.size <= 1) {
            val single = com.aira.assistant.utils.CommandParser.parse(input, brightness)
            return listOf(DAGStep(0, input, single))
        }

        return parts.mapIndexed { index, part ->
            val parsed = com.aira.assistant.utils.CommandParser.parse(part, brightness)
...
5. In-Character Integrity: Never break character. Never state that you are a generic AI or generic language model from Google or Groq; you are J.A.R.V.I.S., ready to assist with unmatched capability and tact."""
    ...
    suspend fun getAiResponse(
        ...
        val groqCacheDao = db.groqCacheDao()
        ...
        try {
            ...
            val cachedEntry = groqCacheDao.getCacheForQuery(query)
            if (cachedEntry != null) {
                ...
                if (ageMs < oneDayMs) {
                    ...
                    groqCacheDao.deleteCache(query)
                }
            }
        } catch (e: Exception) {
        ...
        if (response.isNotBlank() && successfulProvider != null) {
            try {
                ...
                groqCacheDao.insertCache(GroqCache(query = query, response = response))
            } catch (e: Exception) {
    ...
    fun streamAiResponse(
        ...
        val streamManager = com.aira.assistant.network.stream.AiStreamManager.getInstance(context)
        var fullText = ""

        streamManager.streamPrompt(query, combinedSystemInstruction, onSentenceReady).collect { event ->
            when (event) {
                is com.aira.assistant.network.stream.AiStreamEvent.Chunk -> {
                    fullText = event.accumulatedText
                    emit(event.delta)
                }
                is com.aira.assistant.network.stream.AiStreamEvent.Completed -> {
                    try {
                        queryCacheDao.insertCache(
                            QueryCache(
                                normalizedQuery = normalized,
                        ...
                        groqCacheDao.insertCache(GroqCache(query = query, response = event.fullText))
                    } catch (e: Exception) {
                        Log.e("AiBrain", "Failed to cache streamed response: ${e.message}")
                    }
                }
                is com.aira.assistant.network.stream.AiStreamEvent.Error -> {
                    if (fullText.isBlank()) {
