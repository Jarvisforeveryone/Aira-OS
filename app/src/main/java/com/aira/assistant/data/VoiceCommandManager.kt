...
import com.aira.assistant.network.providers.AiBrain
import com.aira.assistant.core.llm.LlamaCppBrain
...
import com.aira.assistant.domain.automation.AiraAutomationEngine
import com.aira.assistant.utils.AssistantActionHandler
...
class VoiceCommandManager(private val context: Context) {
    ...
    companion object {
        ...
        val commandAliases: Map<String, List<String>> = mapOf(
            ...
            "silentMode" to listOf(
                ...
                "silent karo", "silent mode lagao", "phone silent karo", "khamosh mode",
    ...
    suspend fun processCommandIntelligent(userInput: String, viewModel: AssistantActionHandler): Boolean {
        val normalized = userInput.trim().lowercase(Locale.ROOT)
        if (normalized.isBlank()) return false
        ...
        val parsedCmd = com.aira.assistant.utils.CommandParser.parse(userInput)
        if (parsedCmd != null && parsedCmd.type != com.aira.assistant.utils.CommandType.UNKNOWN) {
            Log.i("VoiceCommandManager", "Layer 3 CommandParser Match: '$userInput' -> ${parsedCmd.type}")
            val msg = com.aira.assistant.utils.CommandParser.execute(context, parsedCmd, viewModel)
            viewModel.speakText(msg)
            return true
        }
    ...
    fun executeAction(action: String, viewModel: AssistantActionHandler? = null): String {
    ...
    suspend fun matchAndExecuteCommand(userInput: String, viewModel: AssistantActionHandler): Boolean {
    ...
    private fun executeChainCommand(userInput: String, command: Command, placeholderMap: Map<String, String>, viewModel: AssistantActionHandler) {
    ...
    private fun checkChainConditions(conditionsJson: String, viewModel: AssistantActionHandler): Pair<Boolean, String> {
        return try {
    ...
    private fun performSingleAction(type: String, name: String, paramsJson: String, viewModel: AssistantActionHandler) {
        try {
    ...
    private val llamaCppBrain: com.aira.assistant.core.llm.LlamaCppBrain? by lazy {
        try {
            com.aira.assistant.core.llm.LlamaCppBrain(context)
        } catch (e: Exception) {
    ...
    suspend fun getRoutedAiResponse(
        ...
        val apiBrain = com.aira.assistant.network.providers.AiBrain(context)
        val combinedInstruction = if (systemInstruction.contains("Jarvis")) systemInstruction else "${com.aira.assistant.network.providers.AiBrain.JARVIS_SYSTEM_INSTRUCTION}\n$systemInstruction"
