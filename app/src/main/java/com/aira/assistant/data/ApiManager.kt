...
import com.aira.assistant.network.api.*
...
class ApiManager private constructor(private val context: Context) {
    ...
    fun queryAi(prompt: String, systemInstruction: String? = null): Result<Pair<ApiProviderType, String>> {
        val taskType = com.aira.assistant.utils.TaskDetector.detectTaskType(prompt)
    ...
    private fun resolveModelForTask(provider: ApiProviderType, taskType: com.aira.assistant.utils.TaskType): String {
        val userConfigured = getSelectedModel(provider)
        val defaultList = ApiDefaults.modelsMap[provider] ?: emptyList()

        return when (taskType) {
            com.aira.assistant.utils.TaskType.CODE -> when (provider) {
            ...
            com.aira.assistant.utils.TaskType.CREATIVE -> when (provider) {
    ...
    fun testConnection(provider: ApiProviderType, apiKey: String, model: String): Result<String> {
        ...
        return com.aira.assistant.network.api.ApiTest.testKey(context, provider, apiKey, model)
    }
