...
object DatabaseSchema {
    ...
    object Tables {
        ...
        const val GROQ_CACHE = "groq_cache"
        const val GROK_CACHE = "groq_cache"
    ...
    val MIGRATION_11_12 = object : Migration(11, 12) {
        override fun migrate(db: SupportSQLiteDatabase) {
            createTablesIfNotExist(db)
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_reminders_timestamp` ON `${Tables.REMINDERS}` (`timestamp`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_Memory_createdAt` ON `${Tables.MEMORIES}` (`createdAt`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_Memory_isImportant_createdAt` ON `${Tables.MEMORIES}` (`isImportant`, `createdAt`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_voice_command_logs_timestamp` ON `${Tables.VOICE_COMMAND_LOGS}` (`timestamp`)")
        }
    }

    val MIGRATION_12_13 = object : Migration(12, 13) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE grok_cache RENAME TO groq_cache")
        }
    }
